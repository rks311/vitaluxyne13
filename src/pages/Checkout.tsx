  ...
  return (
    <div className="container py-6 md:py-10 min-h-screen max-w-2xl mx-auto pb-24">
      ...
      <div className="flex items-center gap-2 mb-8">
        {[{ id: 1, label: "Infos", icon: User }, { id: 2, label: "Livraison", icon: Truck }, { id: 3, label: "Confirmer", icon: Check }].map((s, i) => {
          const Icon = s.icon;
          return (
      ...
      <div className="grid md:grid-cols-5 gap-6">
        <div className="md:col-span-3">
          <AnimatePresence mode="wait">
            {step === 1 && (
              <motion.div key="s1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-3">
                ...
                <input placeholder="Nom complet" value={form.name} onChange={(e) => update("name", e.target.value)} className="w-full h-12 rounded-xl bg-card border border-border px-4 text-sm focus:outline-none focus:ring-1 focus:ring-primary" maxLength={100} />
                <input placeholder="Téléphone (ex: 0555123456)" value={form.phone} onChange={(e) => update("phone", e.target.value)} type="tel" className="w-full h-12 rounded-xl bg-card border border-border px-4 text-sm focus:outline-none focus:ring-1 focus:ring-primary" maxLength={15} />
                ...
                <input placeholder="Commune" value={form.commune} onChange={(e) => update("commune", e.target.value)} className="w-full h-12 rounded-xl bg-card border border-border px-4 text-sm focus:outline-none focus:ring-1 focus:ring-primary" maxLength={100} />
                <input placeholder="Adresse complète" value={form.address} onChange={(e) => update("address", e.target.value)} className="w-full h-12 rounded-xl bg-card border border-border px-4 text-sm focus:outline-none focus:ring-1 focus:ring-primary" maxLength={200} />
                <textarea placeholder="Notes supplémentaires (optionnel)" value={form.notes} onChange={(e) => update("notes", e.target.value)} rows={2} className="w-full rounded-xl bg-card border border-border px-4 py-3 text-sm focus:outline-none focus:ring-1 focus:ring-primary resize-none" maxLength={300} />
              </motion.div>
            )}
            {step === 2 && (
              <motion.div key="s2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-3">
                <h2 className="font-heading text-lg font-bold">Mode de livraison</h2>
                {[{ id: "domicile", label: "Livraison à domicile", desc: `Yalidine Express - ${formatPrice(DELIVERY_FEE)}`, icon: "🏠" }, { id: "relais", label: "Point relais Yalidine", desc: "Gratuit - Retrait en agence", icon: "📍" }].map((opt) => (
                  <button key={opt.id} onClick={() => update("delivery", opt.id)} className={`w-full p-4 rounded-xl border text-left transition-all ${form.delivery === opt.id ? "border-primary bg-primary/5 neon-glow" : "border-border bg-card hover:border-primary/30"}`}>
                    <div className="flex items-center justify-between">
                      <div><span className="mr-2">{opt.icon}</span><span className="font-medium text-sm">{opt.label}</span><p className="text-xs text-muted-foreground mt-1">{opt.desc}</p></div>
                      {form.delivery === opt.id && <Check size={18} className="text-primary" />}
                    </div>
                  </button>
                ))}
                <div className="p-4 rounded-xl bg-card border border-border">
                  <div className="flex items-center gap-2 mb-2"><CreditCard size={16} className="text-primary" /><span className="font-medium text-sm">Paiement à la livraison</span></div>
            ...
            {step === 3 && (
              <motion.div key="s3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-3">
                ...
                <div className="p-4 rounded-xl border border-primary/20 bg-primary/5">
                  <div className="flex items-center gap-2 mb-1"><Phone size={14} className="text-primary" /><span className="text-sm font-medium">Confirmation via WhatsApp</span></div>
                  <p className="text-xs text-muted-foreground">Votre commande sera envoyée via WhatsApp et enregistrée.</p>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
          ...
          <div className="flex gap-3 mt-6">
            {step > 1 && <Button variant="outline" onClick={() => setStep(step - 1)} className="border-border rounded-xl">Retour</Button>}
            {step < 3 ? (
              <Button onClick={() => setStep(step + 1)} disabled={!canProceed} className="flex-1 h-12 rounded-xl gradient-primary text-primary-foreground disabled:opacity-50">Suivant</Button>
            ) : (
        ...
        <div className="md:col-span-2 bg-card border border-border rounded-xl p-4 h-fit sticky top-20">
          ...
          <div className="border-t border-border pt-3 space-y-1 text-sm">
            ...
            <div className="flex justify-between font-heading font-bold text-base pt-2 border-t border-border mt-2">
              <span>Total</span><span className="text-primary">{formatPrice(total + (form.delivery === "relais" ? 0 : DELIVERY_FEE))}</span>
            </div>
          </div>
        </div>
      </div>
