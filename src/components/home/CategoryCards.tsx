    ...
    <section className="py-8 md:py-14">
      <div className="container">
        ...
        >
          ...
          <h2 className="font-heading text-xl md:text-2xl font-bold tracking-wide">
            CATÉGORIES
          </h2>
        </motion.div>
      </div>
        ...
        <div className="flex gap-3 overflow-x-auto pb-4 snap-x snap-mandatory scrollbar-hide md:grid md:grid-cols-4 lg:grid-cols-7 md:overflow-visible md:pb-0">
          {categories.map((cat, i) => (
            <motion.div
              ...
              className="snap-start shrink-0 w-[120px] md:w-auto"
            >
              <Link
                to={cat.id === "packs" ? "/catalogue?cat=packs" : `/catalogue?cat=${cat.id}`}
                className="group block relative rounded-2xl overflow-hidden aspect-[3/4] active:scale-95 transition-transform"
              >
                <img
                ...
                <div className="absolute bottom-0 left-0 right-0 p-3 text-center">
                  ...
                  <h3 className="font-heading font-bold text-[11px] md:text-sm text-foreground uppercase tracking-wider">
                    {cat.label}
                  </h3>
                </div>
              </Link>
